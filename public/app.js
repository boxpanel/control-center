...
const SDK_TRAFFIC_LIGHT_SIGNAL_OPTIONS = [
  { value: 0, label: "车检器" },
  { value: 1, label: "红绿灯检测器" }
];

const SDK_EPOLICE_SERIAL_TYPE_OPTIONS = [
  { value: 0, label: "海康车检器协议" },
  { value: 1, label: "海康OEM车检器协议" },
  { value: 2, label: "其它车检器协议" }
];

const SDK_EPOLICE_SNAP_PRE_RECORD_OPTIONS = [
  { value: 0, label: "默认" },
  { value: 1, label: "1" },
  { value: 2, label: "2" },
  { value: 3, label: "3" }
...
const SDK_EPOLICE_NORMAL_PASS_OPTIONS = [
  { value: 0, label: "不抓拍" },
  { value: 1, label: "进1" },
  { value: 2, label: "进1_出1_出2_1" },
  { value: 3, label: "进1_出1_出2_2" }
];

const SDK_EPOLICE_INVERSE_OPTIONS = [
  { value: 0, label: "不抓拍" },
  { value: 1, label: "抓拍" }
];

const SDK_EPOLICE_SPEED_OPTIONS = [
  { value: 0, label: "不抓拍" },
  { value: 1, label: "抓拍" }
];
...
const DEVICE_PREVIEW_ISAPI_SCHEMAS = {
  ...
  namingRules: {
    ...
    mapLoadResult(result = {}, device = null) {
...
async function fetchActivationState() {
  try {
    ...
    return { features: r?.features || {}, fingerprint: r?.fingerprint || "" };
  } catch {
...
async function initActivationUi() {
  const { features: state } = await fetchActivationState();
  ...
  if (els.plateImageDownloadBtn) {
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        btn.textContent = `失败(${error.message.slice(0, 18)})`;
...
if (copyBtn) {
  copyBtn.addEventListener("click", async () => {
    const fpEl = document.getElementById("fingerprintBox");
    if (!fpEl) return;
    const text = fpEl.textContent || "";
    const fp = text.replace(/^设备指纹码：/, "").trim();
    if (!fp) return;
    try {
...
function renderPreviewSelectOptions(options = []) {
  return options
    .map((option) => {
      if (option && typeof option === "object") {
        return `<option value="${option.value ?? ""}">${option.label ?? option.value ?? ""}</option>`;
...
function renderDevicePreviewSdkFtpNamingRows() {
  const selectOptions = renderPreviewSelectOptions(SDK_FTP_PICTURE_ITEM_OPTIONS);
  const rows = [];
...
function renderDevicePreviewSdkFtpTopForm() {
  return [
    ...
    '<label class="devicePreviewFtpField"><span>端口</span><input class="devicePreviewParamInput" type="number" data-isapi-field="ftpPort" /></label>',
    '<label class="devicePreviewFtpField"><span>用户名</span><input class="devicePreviewParamInput" type="text" data-isapi-field="ftpUsername" /></label>',
    '<label class="devicePreviewFtpField"><span>密码</span><input class="devicePreviewParamInput" type="password" data-isapi-field="ftpPassword" /></label>',
    '<label class="devicePreviewFtpField"><span>密码确认</span><input class="devicePreviewParamInput" type="password" data-isapi-field="ftpPasswordConfirm" /></label>',
    '<label class="devicePreviewFtpCheck"><input type="checkbox" data-isapi-field="filterCarPic" /><span>车牌小图不上载</span></label>',
    '</div>',
    '<div class="devicePreviewFtpColumn">',
    `<label class="devicePreviewFtpField"><span>目录结构</span><select class="devicePreviewParamSelect" data-isapi-field="dirLevel">${renderPreviewSelectOptions(SDK_FTP_DIR_LEVEL_OPTIONS)}</select></label>`,
    `<label class="devicePreviewFtpField"><span>一级目录</span><select class="devicePreviewParamSelect" data-isapi-field="topDirMode">${renderPreviewSelectOptions(SDK_FTP_DIR_MODE_OPTIONS)}</select></label>`,
    `<label class="devicePreviewFtpField"><span>二级目录</span><select class="devicePreviewParamSelect" data-isapi-field="subDirMode">${renderPreviewSelectOptions(SDK_FTP_DIR_MODE_OPTIONS)}</select></label>`,
    `<label class="devicePreviewFtpField"><span>三级目录</span><select class="devicePreviewParamSelect" data-isapi-field="threeDirMode">${renderPreviewSelectOptions(SDK_FTP_DIR_MODE_OPTIONS)}</select></label>`,
    `<label class="devicePreviewFtpField"><span>四级目录</span><select class="devicePreviewParamSelect" data-isapi-field="fourDirMode">${renderPreviewSelectOptions(SDK_FTP_DIR_MODE_OPTIONS)}</select></label>`,
    '</div>',
    '</div>',
    '<input type="hidden" data-isapi-field="ftpEnabledRaw" />',
    '<input type="hidden" data-isapi-field="ftpAddressTypeRaw" />',
    '<input type="hidden" data-isapi-field="ftpIndexRaw" />',
    '<input type="hidden" data-isapi-field="topCustomDirRaw" />',
    '<input type="hidden" data-isapi-field="subCustomDirRaw" />',
    '<input type="hidden" data-isapi-field="threeCustomDirRaw" />',
    '<input type="hidden" data-isapi-field="fourCustomDirRaw" />',
    '</div>'
  ].join("");
}

function renderDevicePreviewSdkNamingSection(includeOsd = false) {
  const html = [
    '<div class="devicePreviewFtpSection">',
    '<div class="devicePreviewFtpSectionBar"><span>图片命名规则</span>',
    `<label class="devicePreviewFtpInlineField"><span>分隔符：</span><select class="devicePreviewParamSelect" data-isapi-field="picNameDelimiter">${renderPreviewSelectOptions(SDK_FTP_DELIMITER_OPTIONS)}</select></label>`,
    '</div>',
    '<div class="devicePreviewFtpTableWrap">',
    '<table class="devicePreviewFtpTable">',
    '<thead><tr><th>序号</th><th>命名项</th><th>命名元素</th></tr></thead>',
  ...
  if (includeOsd) {
    html.push(
      '<div class="devicePreviewFtpSection">',
      '<div class="devicePreviewFtpSectionBar"><span>OSD相关信息</span></div>',
      '<div class="devicePreviewFtpTopGrid devicePreviewFtpOsdGrid">',
      '<div class="devicePreviewFtpColumn">',
      '<label class="devicePreviewFtpField"><span>地点</span><input class="devicePreviewParamInput" type="text" data-isapi-field="osdLocation" readonly /></label>',
      '<label class="devicePreviewFtpField"><span>设备编号</span><input class="devicePreviewParamInput" type="text" data-isapi-field="osdDeviceNo" readonly /></label>',
      '<label class="devicePreviewFtpField"><span>方向编号</span><input class="devicePreviewParamInput" type="text" data-isapi-field="osdDirectionNo" readonly /></label>',
      '</div>',
      '<div class="devicePreviewFtpColumn">',
      '<label class="devicePreviewFtpField"><span>路口编号</span><input class="devicePreviewParamInput" type="text" data-isapi-field="osdIntersectionNo" readonly /></label>',
      '<label class="devicePreviewFtpField"><span>监测点1</span><input class="devicePreviewParamInput" type="text" data-isapi-field="osdMonitoringSite1" readonly /></label>',
      '<label class="devicePreviewFtpField"><span>方向描述</span><input class="devicePreviewParamInput" type="text" data-isapi-field="osdDirectionDesc" readonly /></label>',
      '</div>',
      '</div>',
      '<div class="devicePreviewFtpNote">当前SDK尚未返回 OSD 相关信息，先保留与摄像头页面一致的显示区域。</div>',
      '</div>'
    );
  }
...
function renderEpoliceCopyGroup(fieldName) {
  const items = [];
  for (let bit = 1; bit <= 3; bit += 1) {
    items.push(`<label class="devicePreviewTriggerIoItem"><input type="checkbox" data-isapi-bitmask-item="${bit}" ${bit === 1 ? "disabled" : ""} /><span>车道${bit}</span></label>`);
  }
...
function renderMixedLaneViolationField(label, enabledKey, levelKey) {
  return `<label class="devicePreviewTriggerField devicePreviewMixedViolationField"><span>${label}</span><span class="devicePreviewTriggerControl devicePreviewTriggerMixedControl"><input type="checkbox" data-isapi-field="${enabledKey}" /><select class="devicePreviewParamSelect" data-isapi-field="${levelKey}">${renderPreviewSelectOptions(SDK_VIDEO_EPOLICE_LEVEL_OPTIONS)}</select></span></label>`;
}
...
function renderDevicePreviewSdkTriggerConfigControls() {
  return [
    ...
    '<div class="devicePreviewTriggerSectionTitle">混合抓拍配置</div>',
    '<div class="devicePreviewTriggerGrid">',
    '<div class="devicePreviewTriggerLeft">',
    renderTriggerFieldRow("连拍间隔类型", "firstLaneIntervalType", { type: "select", selectOptions: SDK_INTERVAL_TYPE_OPTIONS }),
    renderMixedLaneViolationField("卡口", "firstLaneEmergencyCapEnabled", "mixedCheckpointLevel"),
    renderMixedLaneViolationField("超速", "firstLaneSpeedCapEnabled", "mixedSpeedLevel"),
    renderMixedLaneViolationField("压车道线", "mixedLaneLineEnabled", "mixedLaneLineLevel"),
    '</div>',
    '<div class="devicePreviewTriggerLeft">',
    renderTriggerFieldRow("连拍间隔", "firstLaneInterval1", { type: "number", unit: "ms" }),
    renderMixedLaneViolationField("逆行", "mixedReverseEnabled", "mixedReverseLevel"),
    renderMixedLaneViolationField("低速", "firstLaneLowSpeedCapEnabled", "mixedLowSpeedLevel"),
    renderMixedLaneViolationField("闯禁令", "mixedBanEnabled", "mixedBanLevel"),
    '</div>',
    '</div>',
    '<div class="devicePreviewTriggerSectionTitle">线路和抓拍配置</div>',
    '<div class="devicePreviewTriggerGrid">',
    '<div class="devicePreviewTriggerLeft">',
    renderTriggerFieldRow("线路模式", "sceneMode", { type: "select", selectOptions: SDK_SCENE_MODE_OPTIONS }),
    ...
    renderTriggerFieldRow("测速模式", "speedMode", { type: "select", selectOptions: SDK_SPEED_MODE_OPTIONS }),
    '</div>',
    '</div>',
    '</div>',
    ...
    '<div class="devicePreviewVehicleDetectorOnly devicePreviewTriggerSectionTitle">车检器触发</div>',
    ...
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("车道方向类型", "firstLaneDirectionType", { type: "select", selectOptions: SDK_LANE_DIRECTION_OPTIONS }) + '</div>',
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("叠加车道号", "firstLaneOverlayDriveWay", { type: "number" }) + '</div>',
    '<div class="devicePreviewRadarOnly">' + renderTriggerFieldRow("叠加车道号", "firstLaneOverlayDriveWay", { type: "number" }) + '</div>',
    '<div class="devicePreviewVehicleDetectorOnly">' + renderTriggerFieldRow("车道方向类型", "firstLaneDirectionType", { type: "select", selectOptions: SDK_LANE_DIRECTION_OPTIONS }) + '</div>',
    ...
    '<div class="devicePreviewVehicleDetectorOnly">' + renderTriggerFieldRow("车道属性", "firstLaneLaneType", { type: "select", selectOptions: SDK_LANE_PROPERTY_OPTIONS }) + '</div>',
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("车道属性", "firstLaneLaneType", { type: "select", selectOptions: SDK_LANE_PROPERTY_OPTIONS }) + '</div>',
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("车道行驶方向", "videoEpoliceCarDriveDirect", { type: "select", selectOptions: SDK_VIDEO_EPOLICE_DRIVE_DIRECT_OPTIONS }) + '</div>',
    ...
    '<div class="devicePreviewStandardLaneOnly">' + renderTriggerFieldRow("连拍张数", "firstLaneSnapTimes", { type: "select", selectOptions: SDK_SNAP_TIMES_OPTIONS }) + '</div>',
    '<div class="devicePreviewStandardLaneOnly">' + renderTriggerCheckboxField("应急车道抓拍", "firstLaneEmergencyCapEnabled") + '</div>',
    '<div class="devicePreviewVehicleDetectorOnly">' + renderTriggerCheckboxField("超速抓拍(超速多抓一张)", "firstLaneSpeedCapEnabled") + '</div>',
    '<div class="devicePreviewVehicleDetectorOnly">' + renderTriggerCheckboxField("启用低速抓拍", "firstLaneLowSpeedCapEnabled") + '</div>',
    ...
    '<div class="devicePreviewVehicleDetectorOnly">' + renderTriggerFieldRow("连拍间隔类型", "firstLaneIntervalType", { type: "select", selectOptions: SDK_INTERVAL_TYPE_OPTIONS }) + '</div>',
    '<div class="devicePreviewVehicleDetectorOnly">' + renderLaneIntervalFields() + '</div>',
    ...
    '<div class="devicePreviewVehicleDetectorOnly">' + renderTriggerFieldRow("限速值", "firstLaneSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewVehicleDetectorOnly">' + renderTriggerFieldRow("标志限速", "firstLaneSignSpeed", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewVehicleDetectorOnly">' + renderTriggerFieldRow("大车限高速", "firstLaneCartSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewVehicleDetectorOnly">' + renderTriggerFieldRow("大车标志限速", "firstLaneCartSignSpeed", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewVehicleDetectorOnly">' + renderTriggerFieldRow("小车限低速", "firstLaneLowSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewVehicleDetectorOnly">' + renderTriggerFieldRow("大车限低速", "firstLaneBigCarLowSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("小车标志限速", "firstLaneSignSpeed", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("限速值", "firstLaneSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("小车限低速", "firstLaneLowSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("大车标志限速", "firstLaneCartSignSpeed", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("大车限高速", "firstLaneCartSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("大车限低速", "firstLaneBigCarLowSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
    ...
    '<div class="devicePreviewEpoliceOnly">' + renderTriggerFieldRow("车检器协议类型", "epoliceSerialType", { type: "select", selectOptions: SDK_EPOLICE_SERIAL_TYPE_OPTIONS }) + '</div>',
    '<div class="devicePreviewCardEpoliceOnly">' + renderTriggerFieldRow("车检器协议类型", "epoliceSerialType", { type: "select", selectOptions: SDK_EPOLICE_SERIAL_TYPE_OPTIONS }) + '</div>',
    '<div class="devicePreviewEpoliceOnly">' + renderTriggerFieldRow("闯红灯抓拍逻辑", "epoliceNormalPassProtocol", { type: "select", selectOptions: SDK_EPOLICE_NORMAL_PASS_OPTIONS }) + '</div>',
    '<div class="devicePreviewCardEpoliceOnly">' + renderTriggerFieldRow("闯红灯抓拍逻辑", "epoliceNormalPassProtocol", { type: "select", selectOptions: SDK_EPOLICE_NORMAL_PASS_OPTIONS }) + '</div>',
    '<div class="devicePreviewCardEpoliceOnly">' + renderTriggerFieldRow("正常抓拍逻辑", "epoliceSerialProtocol", { type: "select", selectOptions: SDK_EPOLICE_NORMAL_PASS_OPTIONS }) + '</div>',
    ...
    '<div class="devicePreviewCardEpoliceOnly"><button type="button" class="devicePreviewTriggerAdvancedLink" data-trigger-advanced-toggle="cardEpolice">高级设置</button></div>',
    '<div class="devicePreviewCardEpoliceOnly devicePreviewTriggerAdvancedPanel view-hidden" data-trigger-advanced-panel="cardEpolice">',
    ...
    '<div class="devicePreviewTriggerIoRow"><span>复制车检器协议到车道</span>' + renderEpoliceCopyGroup("epoliceCopyProtocolMask") + '</div>',
    '<div class="devicePreviewTriggerIoRow"><span>参数复制到</span>' + renderEpoliceCopyGroup("epoliceCopyParamMask") + '</div>',
    '</div>',
    ...
    '<div class="devicePreviewVehicleDetectorOnly">' + renderTriggerFieldRow("大车异常超速", "firstLaneCartSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewVehicleDetectorOnly">' + renderTriggerFieldRow("大车异常低速", "firstLaneBigCarLowSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("异常超速", "firstLaneSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("异常低速", "firstLaneLowSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("大车异常超速", "firstLaneCartSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
    '<div class="devicePreviewMixedLaneOnly">' + renderTriggerFieldRow("大车异常低速", "firstLaneBigCarLowSpeedLimit", { type: "number", unit: "km/h" }) + '</div>',
...
function syncDevicePreviewTriggerModeSections() {
  ...
  const showRs485Radar = triggerType === 8;
  ...
  const showLane = showSingleIo || showVehicleDetector || showCapture || showAnyEpolice || showRs485Radar;
  const showRadar = showRs485Radar || showCapture;
  const showSpare = showVehicleDetector;
  const showLaneCount = triggerType === 4 || triggerType === 8 || triggerType === 32 || showAnyEpolice;
  ...
  host.querySelectorAll(".devicePreviewRadarOnly").forEach((node) => node.classList.toggle("view-hidden", !showRs485Radar));
  ...
  host.querySelectorAll(".devicePreviewMixedLaneOnly").forEach((node) => node.classList.toggle("view-hidden", !showCapture));
  ...
  host.querySelectorAll(".devicePreviewStandardLaneOnly").forEach((node) => node.classList.toggle("view-hidden", showSingleIo || showCapture || showCardEpolice || showVideoEpolice));
...
function collectDevicePreviewIsapiControlValues() {
  ...
  host.querySelectorAll("[data-isapi-field]").forEach((field) => {
    if (field.type !== "hidden" && field.closest(".view-hidden")) return;
  ...
  host.querySelectorAll("[data-isapi-bitmask-field]").forEach((group) => {
    if (group.closest(".view-hidden")) return;
...
function fillDevicePreviewIsapiControls(values = {}) {
  ...
  if (!host) return;
  Object.entries(values || {}).forEach(([key, value]) => {
    host.querySelectorAll(`[data-isapi-field="${key}"]`).forEach((field) => {
      setPreviewControlValue(field, value);
    });
  });
  host.querySelectorAll("[data-isapi-bitmask-field]").forEach((group) => {
...
async function runDevicePreviewIsapiRequest(methodOverride = "") {
  ...
  try {
    if (protocol === "onvif") {
      ...
      if (els.devicePreviewIsapiHint) {
        els.devicePreviewIsapiHint.textContent = `${preset.label}读取成功`;
      }
    } else {
  ...
  try {
    ...
    if (schemaKey === "ftpConfig") {
      ...
      if (response && (response.ok || response.success)) {
        ...
        throw new Error(response?.error || "SDK 命名规则获取失败");
      }
